...
echo "${PREFIX} curled latest installer" >> ./RALVIN.log

chmod +x ./virtualmin-installer.sh
./virtualmin-installer.sh --hostname "$1" --force
echo "${PREFIX} completed install with hostname $1" >> ./RALVIN.log

if [[ ! -z "$2" && ! -z "$3" ]]; then
	sudo /usr/libexec/webmin/changepass.pl /etc/webmin "$2" "$3"
	echo "${PREFIX} password updated for Virtualmin user $2" >> ./RALVIN.log
fi